...
function fairFunctionList(symbols, region, max = 20000) {
  ...
  if (total <= max) {
    ...
    // Stratified sample across the entire address range. Endpoints are included.
    const seen = new Set();
...
/** Only a string with a proven code xref is a finding. */
...
export function autoNextSteps(report) {
  ...
  if (report.findings?.some((f) => f.id === 'endpoint')) steps.push({ code: 'check-endpoint', detail: null });
  if (report.findings?.some((f) => f.id === 'anticheat')) steps.push({ code: 'check-anticheat', detail: null });
