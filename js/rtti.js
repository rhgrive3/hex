...
function readSubstitution(p) {
  ...
  if (p.i >= p.s.length || p.s[p.i] !== '_') throw new Error('invalid-substitution');
  p.i++;
  if (idx && !/^[0-9A-Z]+$/i.test(idx)) throw new Error('invalid-substitution');
  const n = idx === '' ? 0 : parseInt(idx, 36) + 1;
  if (!Number.isSafeInteger(n) || n < 0 || n >= p.subs.length || !p.subs[n]) {
    // Never invent a plausible type/name for an invalid ABI substitution. The
    // caller will return null and retain the original mangled symbol.
