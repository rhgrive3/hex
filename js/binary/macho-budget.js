...
/**
 * One parser-wide decoded-output/work budget. Compact LINKEDIT/load-command
 * metadata must never amplify into an unbounded JS object graph.
 */
export function createMachOMetadataBudget(image, options = {}) {
  ...
  const used = {
    ...
    stringBytes: 0,
    operations: 0,
    warnings: Math.min(image.warnings?.length || 0, limits.warnings),
  ...
  const stop = (reason) => {
    markMachOMetadataPartial(image, `budget:${reason}`);
    return false;
  };
  const budget = {
    limits, used, signal,
...
export function ensureMachOMetadataBudget(image, budget = null) {
  return budget || createMachOMetadataBudget(image);
}
