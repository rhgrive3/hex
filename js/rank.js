...
function fairPrefix(values, max) {
  ...
  // Keep strongest evidence, but reserve half the budget for evenly spaced tail
  // ranks so relevant strings at rank 301+ can still seed candidates.
...
export function rankCandidates({ goal, strings, program, symbols, region, limit = 40, vendors = null }) {
  ...
  for (const ms of selectedStrings.values) {
    ...
    for (const u of users) {
      ...
      if (factor > 0) addReason(c, REASON.STRING_REF, POINTS[REASON.STRING_REF] * ms.score * factor, { text: ms.text, addr: ms.addr, site: u.site, term: ms.term, refKind: u.kind, users: users.length, complete: users.complete !== false });
    }
  }
  ...
  if (hasProgram) {
    ...
    for (const seed of seeds) {
      ...
      // Unknown end means "we do not know this function body", not "scan until
      // EOF". Only caller edges are safe without a proven function range.
      if (range && range.start === seed.addr && range.end != null) {
        for (const callee of program.calleesOf(range.start, range.end, 40)) {
          ...
          if (seed.score >= 20 && !tainted(seed.name)) {
            ...
            if (c && c !== seed && !c.reasons.some((r) => r.code === REASON.CALLED_BY_MATCH)) addReason(c, REASON.CALLED_BY_MATCH, POINTS[REASON.CALLED_BY_MATCH], { from: seed.addr, fromName: seed.name, site: callee.site });
          }
        }
      ...
      for (const caller of program.callersOf(seed.addr, 40)) {
        ...
        if (seed.score >= 20 && !tainted(seed.name)) {
          ...
          if (c && c !== seed && !c.reasons.some((r) => r.code === REASON.CALLS_MATCH)) addReason(c, REASON.CALLS_MATCH, POINTS[REASON.CALLS_MATCH], { to: seed.addr, toName: seed.name, site: caller.site });
        }
      }
    }
  }
  ...
  for (const c of byAddr.values()) {
    ...
    if (expects.numeric && stats.numeric > 0) addReason(c, REASON.NUMERIC, Math.min(POINTS[REASON.NUMERIC], 4 * stats.numeric), { mul: stats.mul, div: stats.div, fmul: stats.fmul, farith: stats.farith });
    if (expects.store && stats.store > 0) addReason(c, REASON.STORE, POINTS[REASON.STORE], { n: stats.store });
    if (expects.compare && stats.cmp > 0) addReason(c, REASON.COMPARE, POINTS[REASON.COMPARE], { n: stats.cmp });
    const called = program.callCountOf(c.addr);
    if (called >= 3) addReason(c, REASON.POPULAR, POINTS[REASON.POPULAR], { n: called });
    c.calledCount = called;
    if (stats.total > 1500) addReason(c, REASON.SIZE_PENALTY, POINTS[REASON.SIZE_PENALTY], { n: stats.total });
  }
  ...
  for (const c of byAddr.values()) {
    ...
    if (!vendor) continue;
    c.vendor = vendor;
    if (vendorConflicts(goal.id, vendor)) addReason(c, REASON.VENDOR, POINTS[REASON.VENDOR], { vendor: vendor.vendor, kind: vendor.kind, where: vendor.where, name: vendor.name || null, text: vendor.text || null });
  }
  ...
  return { candidates: out.slice(0, limit), matchedStrings, notes, total: out.length, matchedStringsComplete: selectedStrings.complete };
}
