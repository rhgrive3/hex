...
export class AnalysisScheduler {
  constructor({ store, maxConcurrency=2, starvationInterval=8, defaultBudget={} }={}) {
    ...
    this.store=store; this.maxConcurrency=maxConcurrency; this.starvationInterval=Math.max(1,Number(starvationInterval)||8); this.defaultBudget=defaultBudget;
    this.inflight=new Map(); this.queue=[]; this.running=0; this.dispatchEpoch=0; this.dag=new Map(); this.states=new Map();
    this.metrics={ requests:0,cacheHits:0,producerInvocations:0,coalescedRequests:0,queueOperations:0,completedJobs:0,failedJobs:0,cancelledJobs:0,budgetExhaustions:0,dependencyFailures:0,cycleErrors:0,dependencyIdentityErrors:0 };
  }

  request(request) { return this.#request(request,[]); }
  ...
  #request(request, ancestry) {
    ...
    if (existing) { this.metrics.coalescedRequests++; return waitForConsumer(existing.promise,request.signal); }
    ...
    const task={ artifactId,descriptor,request,controller,priority:priorityValue(request.priority),enqueuedEpoch:null,state:'waiting-dependency',resolve:null,reject:null,promise:null };
    task.promise=this.#executeDependenciesThenQueue(task,[...ancestry,artifactId]).finally(()=>{
  ...
  async #executeDependenciesThenQueue(task, ancestry) {
    ...
    catch (error) { this.metrics.dependencyIdentityErrors++; this.states.set(task.artifactId,'failed'); throw error; }
    const dependencyIds=dependencies.map((dependency)=>String(dependency.descriptor.artifactId));
    ...
    try {
      ...
      if (error?.name==='AbortError'||task.controller.signal.aborted) { this.metrics.cancelledJobs++; this.states.set(task.artifactId,'cancelled'); throw abortError(task.controller.signal); }
      if (error instanceof SchedulerCycleError || error instanceof SchedulerDependencyIdentityError) throw error;
      this.metrics.dependencyFailures++; this.states.set(task.artifactId,'failed'); throw new SchedulerDependencyError(task.artifactId,error);
    }
    if (task.controller.signal.aborted) { this.metrics.cancelledJobs++; this.states.set(task.artifactId,'cancelled'); throw abortError(task.controller.signal); }
    ...
    return new Promise((resolve,reject)=>{
      task.resolve=resolve; task.reject=reject; task.dependencyResults=dependencyResults; task.state='ready'; task.enqueuedEpoch=this.dispatchEpoch;
      this.states.set(task.artifactId,'ready'); this.queue.push(task); this.metrics.queueOperations++; this.#pump();
    });
  }

  #effectivePriority(task) { const age=Math.max(0,this.dispatchEpoch-task.enqueuedEpoch); return Math.max(0,task.priority-Math.floor(age/this.starvationInterval)); }
  #pump() {
    while (this.running<this.maxConcurrency&&this.queue.length) {
      this.queue.sort((a,b)=>{ const pa=this.#effectivePriority(a),pb=this.#effectivePriority(b); return pa-pb||a.artifactId.localeCompare(b.artifactId); });
      const task=this.queue.shift(); this.metrics.queueOperations++; this.dispatchEpoch++;
      if (task.controller.signal.aborted) { this.metrics.cancelledJobs++; this.states.set(task.artifactId,'cancelled'); task.reject(abortError(task.controller.signal)); continue; }
      this.running++; task.state='running'; this.states.set(task.artifactId,'running');
      this.#run(task).finally(()=>{ this.running--; this.#pump(); });
    }
  }

  async #run(task) {
    const signal=task.controller.signal;
    const budget=task.request.budget instanceof ResourceBudget ? task.request.budget : new ResourceBudget({...this.defaultBudget,...(task.request.budget||{})},{signal});
    try {
      ...
      budget.checkCancelled(); this.metrics.completedJobs++; this.states.set(task.artifactId,'completed');
      task.resolve({...published,state:'completed',reused:false,budget:budget.snapshot()});
    } catch (error) {
      if (error instanceof BudgetExceededError) { this.metrics.budgetExhaustions++; this.states.set(task.artifactId,'budget-exhausted'); }
      else if (error?.name==='AbortError'||signal.aborted) { this.metrics.cancelledJobs++; this.states.set(task.artifactId,'cancelled'); }
      else { this.metrics.failedJobs++; this.states.set(task.artifactId,'failed'); }
      task.reject(error);
    }
  }
  ...
  cancel(artifactId, reason=new DOMException('Cancelled','AbortError')) {
    ...
    if (index>=0) { this.queue.splice(index,1); this.metrics.queueOperations++; this.metrics.cancelledJobs++; this.states.set(task.artifactId,'cancelled'); task.reject(reason); }
    return true;
  }
  ...
  stats() { return Object.freeze({ schedulerVersion:ANALYSIS_SCHEDULER_VERSION,running:this.running,queued:this.queue.length,inflight:this.inflight.size,dagNodes:this.dag.size,...this.metrics }); }
}
