...
class IndexedMinHeap {
  ...
  #swap(a,b) {
    ...
    this.items[a]=y; this.items[b]=x;
    this.indices.set(y.artifactId,a); this.indices.set(x.artifactId,b);
  }
  #up(index) {
    while (index>0) {
  ...
  push(item) {
    if (this.indices.has(item.artifactId)) throw new Error('scheduler-queue-duplicate');
    const index=this.items.length;
    this.items.push(item); this.indices.set(item.artifactId,index); this.#up(index);
  }
  pop() {
    if (!this.items.length) return null;
    const root=this.items[0]; this.remove(root.artifactId); return root;
  }
  remove(artifactId) {
    const index=this.indices.get(String(artifactId));
    if (index==null) return null;
    const removed=this.items[index];
    const last=this.items.pop();
    this.indices.delete(removed.artifactId);
    if (index<this.items.length) {
      this.items[index]=last; this.indices.set(last.artifactId,index);
      this.#up(index); this.#down(this.indices.get(last.artifactId));
    }
    return removed;
  }
}
...
export class AnalysisScheduler {
  constructor({ store, maxConcurrency=2, starvationInterval=8, defaultBudget={} }={}) {
    ...
    this.starvationInterval=Number(starvationInterval);
    ...
    this.metrics={
      requests:0,cacheHits:0,producerInvocations:0,coalescedRequests:0,queueOperations:0,
      completedJobs:0,failedJobs:0,cancelledJobs:0,cancelledConsumers:0,orphanCancellations:0,
      budgetExhaustions:0,dependencyFailures:0,producerFailures:0,storageFailures:0,
      cycleErrors:0,cycleChecks:0,cycleTraversalSteps:0,dependencyIdentityErrors:0,maxObservedRunning:0,
    };
  }
  ...
  #request(request, ancestry, parentSignal) {
    ...
    const task={
      artifactId,descriptor,request,controller,priority:priorityValue(request.priority),enqueuedEpoch:null,orderKey:null,
      state:'waiting-dependency',phase:'dependency',queueResolve:null,queueReject:null,promise:null,settled:false,consumerCount:0,
    };
  ...
  async #execute(task, ancestry) {
    ...
    try { dependencies=canonicalDependencies(task.request,task.artifactId); }
      ...
      if (error instanceof SchedulerDependencyIdentityError) this.metrics.dependencyIdentityErrors++;
      throw error;
    }
  ...
  async #waitForDependencies(task, dependencies, ancestry) {
    ...
    else task.controller.signal.addEventListener('abort',onParentAbort,{once:true});
  ...
  #registerDag(artifactId, dependencyIds) {
    ...
    for (const dependencyId of ids) {
      ...
      if (dependencyId===artifactId) {
        this.metrics.cycleErrors++;
        throw new SchedulerCycleError([artifactId,artifactId]);
      }
      const path=this.#pathBetween(dependencyId,artifactId);
      if (path) {
        this.metrics.cycleErrors++;
        throw new SchedulerCycleError([artifactId,...path]);
      }
    }
  ...
  #pathBetween(start,target) {
    ...
    return new Promise((resolve,reject)=>{
      ...
      task.queueResolve=resolve;
      task.queueReject=reject;
  ...
  #pump() {
    while (this.running<this.maxConcurrency&&this.queue.size) {
      ...
      this.running++;
      this.metrics.maxObservedRunning=Math.max(this.metrics.maxObservedRunning,this.running);
      task.state='running';
      task.phase='producer';
  ...
  async #run(task) {
    ...
    const budget=task.request.budget instanceof ResourceBudget
      ? task.request.budget.withSignal(signal)
    ...
    const published=await this.store.publish(task.descriptor,payload,{
      signal,completeness:task.request.completeness??'complete',validate:task.request.validate,creation:task.request.creation,
    });
    budget.checkCancelled();
  ...
  stats() {
    return Object.freeze({
      ...
      starvationPolicy:'virtual-deadline-v1',
      starvationInterval:this.starvationInterval,
      running:this.running,
      queued:this.queue.size,
      inflight:this.inflight.size,
      activeConsumers:this.activeConsumers,
      dagNodes:this.dag.size,
      dagEdges:this.dagEdgeCount,
      queueComparisons:this.queue.comparisons,
      producerInvocationCount:this.metrics.producerInvocations,
      ...this.metrics,
    });
  }
}
