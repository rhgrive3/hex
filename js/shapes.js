...
function keyOf(identity, offset, site, role = 'target') {
  // Without object provenance, never aggregate two physical locations merely
  // because they share +0x20. Site-scoped unknowns sacrifice recall rather than
  // fabricating a cross-object behavioural identity.
  return identity != null ? `${role}:${identity}:${offset}` : `${role}:unknown:${String(site)}:${offset}`;
}
...
/**
 * Aggregate by object-provenance + displacement, never by displacement alone.
...
export function foldShapes(scan) {
  ...
  for (let i = 0; i < scan.count; i++) {
    ...
    if (amtKind[i] === AMOUNT.FIELD) {
      ...
      const sourceKey = amountIdentity != null ? `${amountIdentity}:${a}` : `unknown:${String(addr[i])}:${a}`;
  ...
  for (let i = 0; i < scan.count; i++) {
    ...
    const againstKey = targetIdentity != null ? `${targetIdentity}:${disp[i]}` : `unknown:${String(addr[i])}:${disp[i]}`;
