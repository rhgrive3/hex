...
import { DynamicTaskGraph } from './task-graph/task-graph-scheduler.js';
...
export async function startParentDevWorkerRuntime(options = {}) {
  ...
  try {
    ...
    const workerPool = options.workerPool || new MultiTabWorkerPool({ openTab: options.openTab, BroadcastChannelRef: options.BroadcastChannelRef || globalThis.BroadcastChannel, cryptoRef: options.cryptoRef || globalThis.crypto, location: locationRef, now: options.now, sleep: options.sleep });
    const taskGraph = options.taskGraph || new DynamicTaskGraph({ pool: workerPool, now: options.now, sleep: options.sleep });
    return Object.freeze({
      role:'supervisor', mode:'task-graph-capable', enabled:true, tabNodeId:node.tabNodeId, coordinator, skillRegistry, workerPool, taskGraph,
      ...
      taskGraphCreate:(args)=>taskGraph.create(args), taskGraphAdd:(args)=>taskGraph.add(args), taskGraphReplan:(args)=>taskGraph.replan(args), taskGraphStart:()=>taskGraph.start(), taskGraphStatus:()=>taskGraph.status(), taskGraphAwait:(_args,opts={})=>taskGraph.awaitCompletion({signal:opts.signal}), taskGraphCancel:()=>taskGraph.cancel(),
      close(){ void taskGraph.cancel().finally(()=>workerPool.close()); coordinator.close(); },
    });
...
function disabledRuntime({node,error}) {
  ...
  return Object.freeze({
    role:'supervisor',mode:'task-graph-capable',enabled:false,tabNodeId:node.tabNodeId,error:Object.freeze({code,message}),
    ...
    taskGraphCreate:fail,taskGraphAdd:fail,taskGraphReplan:fail,taskGraphStart:fail,taskGraphStatus:fail,taskGraphAwait:fail,taskGraphCancel:fail,
    close(){},
  });
}
