...
export function matchFormulas(steps, labels, vg) {
  const claimed = new Map();          // 数 → その数を書いている文言
  for (const l of labels) {
   ...
   */
  const distinct = new Set(hits.map((h) => h.value.toString())).size;
...
export function findSink(model, vg, acc) {
  ...
  for (const insn of insns) {
    ...
    for (const w of writesByRow.get(insn.row) || []) {
      ...
      return { kind: 'store', row: insn.row, address: insn.address, write: w, clamps };
    }
    /* 呼び出しの引数 */
    ...
    if (insn.isReturn) {
      ...
      if (v && contains(v, cur)) return { kind: 'return', row: insn.row, address: insn.address, clamps };
    }
    /* 途中の作り変え。上限・下限で止めているなら覚えて、追跡は続ける。 */
    const defs = vg.defs.get(insn.row);
    if (!defs) continue;
    for (const reg of Object.keys(defs)) {
      const v = defs[reg];
      if (!v || !contains(v, cur)) continue;
      if (v.k === 'sel') clamps.push({ row: insn.row, address: insn.address, cc: v.cc, expr: v });
      cur = v;
      break;
    }
  }
  if (clamps.length) return { kind: 'clamped', clamps, certain: true };

  /*
   * 値そのものを追えなくなった（分岐の合流で前提が切れた）。
   * それでも、そのレジスタが直後に上限・下限で止められているなら、
   * 「そこで止めている」ことは命令から言える。値の同一性までは言えないので、
   * certain: false を付けて、そうと分かる形で返す。
   */
  const near = insns.filter((i) => i.row > last.row && i.row - last.row <= 96);
  for (const insn of near) {
    ...
    return {
      kind: 'clamped', certain: false,
      clamps: [{ row: insn.row, address: insn.address, cc: null, expr: null }],
    };
  }
  return null;
}
