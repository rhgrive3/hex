...
export function createDevWorkerToolSurface(client) {
  ...
  const required = ['discover', 'claim', 'createChat', 'send', 'observe', 'followup', 'nudge', 'stop', 'result', 'release', 'waitEvent'];
  ...
  const handlers = new Map([
    ...
    [DEV_WORKER_TOOL.SEND, (args = {}) => client.send({ ...args, instruction: buildDevWorkerInstruction(args.instruction) })],
  ...
  return Object.freeze({
    ...
    execute(name, args = {}) { const handler = handlers.get(String(name || '')); if (!handler) throw new TypeError(`Unavailable Dev Worker tool: ${name}`); return handler(args); },
    waitEvent(events, options = {}) { return client.waitEvent({ events, runId: options.runId ?? null }, options); },
  });
}
