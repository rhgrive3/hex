...
export const DEV_BOOTSTRAP_EXTENSION = deepFreeze({
  apiVersion: DEV_EXTENSION_API_VERSION,
  id: 'dev.bootstrap-observer',
  version: DEV_BOOTSTRAP_EXTENSION_VERSION,
  requiresReload: true,
  capabilities: [{ name: DEV_BOOTSTRAP_CAPABILITY, kind: 'identity' }],
  integrity: DEV_BOOTSTRAP_EXTENSION_INTEGRITY,
});

export function createDevBootstrapCheckpoint(value) {
  assertPlainObject(value, 'Dev bootstrap checkpoint');
  assertExactKeys(value, CHECKPOINT_KEYS, 'Dev bootstrap checkpoint');
  return deepFreeze({
    runId: nonEmpty(value.runId, 'runId'),
    goal: nonEmpty(value.goal, 'goal'),
    decisionPolicy: assertDevDecisionPolicy(value.decisionPolicy),
    supervisorSessionKey: nonEmpty(value.supervisorSessionKey, 'supervisorSessionKey'),
    chatgptConversationId: nonEmpty(value.chatgptConversationId, 'chatgptConversationId'),
    pendingTask: cloneJson(value.pendingTask, 'pendingTask'),
    expectedCommit: assertCommit(value.expectedCommit),
    expectedBuildId: assertBuildId(value.expectedBuildId),
    expectedExtensionVersion: nonEmpty(value.expectedExtensionVersion, 'expectedExtensionVersion'),
  });
}

export function createDevBootstrapHandoff(checkpoint) {
  const value = createDevBootstrapCheckpoint(checkpoint);
  return deepFreeze({ runId: value.runId, supervisorSessionKey: value.supervisorSessionKey, chatgptConversationId: value.chatgptConversationId, checkpoint: value });
}

export function verifyDevBootstrapIdentity(checkpoint, activeIdentity, extensionVersion) {
  const expected = createDevBootstrapCheckpoint(checkpoint);
  assertPlainObject(activeIdentity, 'active Dev runtime identity');
  const actual = Object.freeze({
    commit: assertCommit(activeIdentity.commit),
    buildId: assertBuildId(activeIdentity.buildId),
    extensionVersion: nonEmpty(extensionVersion, 'active extension version'),
  });
  const mismatches = [];
  if (actual.commit !== expected.expectedCommit) mismatches.push('commit');
  if (actual.buildId !== expected.expectedBuildId) mismatches.push('buildId');
  if (actual.extensionVersion !== expected.expectedExtensionVersion) mismatches.push('extensionVersion');
  if (mismatches.length) {
    const error = new Error(`Dev bootstrap identity mismatch: ${mismatches.join(', ')}.`);
    error.code = 'dev-bootstrap-identity-mismatch';
    error.mismatches = Object.freeze([...mismatches]);
    throw error;
  }
...
export class DevExtensionLoader {
  ...
  get toolCallActive() { return this.#toolCallDepth > 0; }
  get activeVersion() { return this.#active?.version || null; }
  get stagedVersion() { return this.#staged?.version || null; }
  ...
  async stage(extension) {
    ...
    if (!constantTimeTextEqual(actual, normalized.integrity)) {
      const error = new Error('Dev extension integrity verification failed.');
      error.code = 'dev-extension-integrity-mismatch';
      throw error;
    }
    this.#staged = normalized;
    return Object.freeze({ id: normalized.id, version: normalized.version, integrity: normalized.integrity, verified: true });
  }

  activateAtSafeBoundary({ checkpoint, activeIdentity, reinitialized = false } = {}) {
    if (this.toolCallActive) {
      const error = new Error('Dev extension activation is forbidden during a tool call.');
      error.code = 'dev-extension-tool-call-active';
      throw error;
    }
    ...
    if (verifiedCheckpoint.expectedExtensionVersion !== this.#staged.version) {
      const error = new Error('Staged Dev extension version does not match the bootstrap checkpoint.');
      error.code = 'dev-extension-version-mismatch';
      throw error;
    }
    if (this.#staged.requiresReload && !reinitialized) {
      return deepFreeze({ status: 'reload-required', reason: 'extension-reinitialize', expectedExtensionVersion: this.#staged.version, handoff: createDevBootstrapHandoff(verifiedCheckpoint) });
    }
    const verifiedIdentity = verifyDevBootstrapIdentity(verifiedCheckpoint, activeIdentity, this.#staged.version);
    this.#active = this.#staged;
    this.#activeIdentity = verifiedIdentity;
  ...
  invoke(name) {
    if (!this.#active) throw new Error('No Dev extension is active.');
    const capability = this.#active.capabilities.find((item) => item.name === String(name || ''));
    if (!capability) throw new Error(`Unknown Dev extension capability: ${name}`);
    if (capability.kind !== 'identity') throw new Error(`Unsupported Dev extension capability kind: ${capability.kind}`);
...
function normalizeExtension(value) {
  assertPlainObject(value, 'Dev extension');
  assertExactKeys(value, EXTENSION_KEYS, 'Dev extension');
  if (value.apiVersion !== DEV_EXTENSION_API_VERSION) throw new TypeError(`Unsupported Dev extension API version: ${value.apiVersion}`);
  if (typeof value.requiresReload !== 'boolean') throw new TypeError('requiresReload must be boolean.');
  if (!Array.isArray(value.capabilities) || value.capabilities.length === 0) throw new TypeError('Dev extension capabilities are required.');
  const seen = new Set();
  const capabilities = value.capabilities.map((item) => {
    assertPlainObject(item, 'Dev extension capability');
    assertExactKeys(item, CAPABILITY_KEYS, 'Dev extension capability');
    const capability = { name: nonEmpty(item.name, 'capability.name'), kind: nonEmpty(item.kind, 'capability.kind') };
    if (seen.has(capability.name)) throw new TypeError(`Duplicate Dev extension capability: ${capability.name}`);
    seen.add(capability.name);
    if (capability.kind !== 'identity') throw new TypeError(`Unsupported Dev extension capability kind: ${capability.kind}`);
    return capability;
  });
...
function canonicalExtensionPayload(value) {
  return JSON.stringify({ apiVersion: value.apiVersion, id: value.id, version: value.version, requiresReload: value.requiresReload, capabilities: value.capabilities.map(({ name, kind }) => ({ name, kind })) });
}
async function sha256Hex(text) {
  if (!globalThis.crypto?.subtle) throw new Error('WebCrypto SHA-256 is unavailable.');
  const digest = new Uint8Array(await globalThis.crypto.subtle.digest('SHA-256', new TextEncoder().encode(String(text))));
