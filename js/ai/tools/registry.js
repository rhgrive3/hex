...
export class ToolRegistry {
  ...
  assertScope(tool, args, scope) {
    if (scope !== 'auto' && !tool.scopeSupport.includes(scope)) {
      throw new AIError('scope_violation', `${tool.name} is outside the explicit ${scope} scope.`);
    }
    if (typeof this.context.scopeAllowsTool === 'function' && !this.context.scopeAllowsTool(scope, tool.name, args)) {
      throw new AIError('scope_violation', `${tool.name} was rejected by the local scope boundary.`);
    }
  }

  async assertAddresses(args, scope) {
    for (const address of collectAddresses(args)) {
      if (typeof this.context.addressExists === 'function' && !await this.context.addressExists(address)) {
        throw new AIError('invalid_tool_call', `Address does not exist: ${address}`);
      }
      if (scope !== 'auto' && typeof this.context.scopeContainsAddress === 'function' && !await this.context.scopeContainsAddress(scope, address)) {
        throw new AIError('scope_violation', `Address ${address} is outside ${scope} scope.`);
      }
    }
  }
...
export function createHexToolRegistry(context = {}, options = {}) {
  ...
  const functionScopes = ['auto', 'selection', 'function', 'neighborhood', 'binary', 'project'];
  ...
  }, { cost: 'medium', scopeSupport: allReadScopes });
...
async function fieldAccess(legacy, method, { functionAddress, field, limit = 100 }) {
  const value = await legacy[method](functionAddress, field);
  return boundedResult(value, limit);
}
...
async function runtimeObservations(context, { functionAddress, limit = 100 }) {
  ...
  const evidence = (session?.evidence || session?.observations || []).slice(-limit);
  return { results: evidence, returned: evidence.length, verified: evidence.length > 0 };
}
...
async function binaryDiff(context, limit) {
  const value = typeof context.getBinaryDiff === 'function' ? await context.getBinaryDiff() : context.binaryDiff;
  return boundedResult(value || { results: [] }, limit);
}

function collectAddresses(value, parentKey = '') {
  const out = [];
  if (!value || typeof value !== 'object') return out;
  for (const [key, item] of Object.entries(value)) {
    if ((ADDRESS_KEYS.has(key) || /Address$/.test(key)) && typeof item === 'string' && addressText(item)) out.push(addressText(item));
    else if (item && typeof item === 'object') out.push(...collectAddresses(item, key));
  }
  return out;
}
...
function compactModelData(result) {
  ...
  const out = {};
  for (const key of ['query', 'address', 'functionAddress', 'name', 'found', 'verified', 'total', 'returned', 'truncated', 'reason', 'summary', 'engine']) {
    if (result[key] != null) out[key] = result[key];
  }
  for (const key of ['results', 'sites', 'functions', 'updates', 'nodes', 'paths', 'blocks', 'callers', 'callees']) {
    if (Array.isArray(result[key])) out[key] = result[key].slice(0, 20);
  }
...
function abortError(signal) {
  return signal?.reason === 'timeout'
    ? new AIError('budget_exhausted', 'The tool execution exceeded the turn timeout.')
    : new AIError('cancelled', 'AI investigation was cancelled.');
}
