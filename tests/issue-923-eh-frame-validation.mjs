...
function makeFixture({
  ...
  // .eh_frame_hdr: absolute udata4 encodings keep the fixture independent of relocation bases.
  let p = HEADER_OFFSET;
  ...
  // One CIE: version 1, augmentation "zR", FDE encoding = absolute udata4.
  p = EH_FRAME_OFFSET;
  u32(p, 13); p += 4;
  u32(p, 0); p += 4;
  ...
  for (const entry of entries) {
    ...
    u32(off, 13);
    const pointerFieldAddress = entry.fdeAddress + 4n;
    u32(off + 4, pointerFieldAddress - EH_FRAME_ADDR);
  ...
  const rawHeaderSection = { name:'.eh_frame_hdr', addr:HEADER_ADDR, offset:0n, size:0x40n };
  return { r:new ByteView(bytes, { littleEndian:true }), image, rawHeaderSection };
}

function parse(options) {
  const fixture = makeFixture(options);
  parseEhFrameHeader(fixture.r, fixture.rawHeaderSection, fixture.image, 64, null);
...

{
  ...
  assert.equal(image.functions.length, 0, 'unmapped eh_frame_ptr must suppress unwind seeds');
  assert.equal(image.metadata.ehFrameHeader.validation, 'invalid');
}

{
  const image = parse({ entries:[{ initial:0x1010n, fdeAddress:0xdead1000n }] });
  assert.equal(image.functions.length, 0, 'unmapped FDE pointer must not authorize a function seed');
...

{
  ...
  assert.equal(image.functions.length, 0, 'FDE range escaping executable mapping must not become a function seed');
}

{
  ...
  assert.equal(image.functions.length, 0, 'unsorted binary-search table must fail closed for header-derived seeds');
