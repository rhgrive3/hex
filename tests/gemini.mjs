...
await test('WorkerはGeminiのSSEを同一オリジンのクライアントへ中継する', async () => {
  ...
  try {
    const request = new Request('https://example.test/api/gemini', {
      method: 'POST',
    ...
    const response = await worker.fetch(request, { GEMINI_API_KEY: 'not-exposed', ASSETS: { fetch: () => new Response('asset') } });
...
await test('入力サイズとthinking levelの検証を行う', () => {
