...
for (const bits of [8, 16, 32, 64]) {
  const x = expr.variable('x', bits, false), y = expr.variable('y', bits, false);
  const corpus = [
    expr.binary('add', expr.binary('add', x, expr.constant(5, bits, false), bits, false), expr.constant(7, bits, false), bits, false),
    expr.binary('and', expr.binary('and', x, expr.constant(0xf5, bits, false), bits, false), expr.constant(0x3f, bits, false), bits, false),
    expr.binary('or', expr.binary('or', x, expr.constant(0x10, bits, false), bits, false), expr.constant(0x20, bits, false), bits, false),
    expr.select(expr.compare('gt', x, y, false), x, y, bits, false),
    expr.binary('mul', x, expr.constant(8, bits, false), bits, false),
  ];
  for (const original of corpus) {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      assert.equal(u(a, bits), u(b, bits), `${bits}-bit mismatch ${printExpression(original)} -> ${printExpression(rewritten)}`);
      checks++;
    }
  }
}
assert.ok(checks > 2000);
console.log(`decompiler rewrite property tests: ${checks} semantic checks PASS`);
