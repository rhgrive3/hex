...
function stackRootProvider(request) {
  ...
  if (identity?.kind !== 'register' || String(identity.registerId) !== 'sp') return null;
  return { kind:'stack-like', addressSpace:request.expectedAddressSpace ?? 'memory', baseOffset:0, linearOffsets:true, rootIdentity:{kind:'diag-stack', storageClass:'function-local-stack'} };
}
...
try {
  ...
  }
  {
    ...
    const pipeline = buildSemanticV2CompatibilityPipeline({
      ...
      decoderSemanticVersion:'diag-o0-direct',
      binaryId:'diag-o0-binary', sliceId:'diag-o0-slice', addressWidthBits:64,
      canonicalStartIdentity:{address:BASE}, entryBlockKey:'b0', rootDescriptorProvider:stackRootProvider,
      blocks:[{ key:'b0', startAddress:BASE, instructions:model.instructions.map((decoded) => ({decoded,address:decoded.address,size:4,mode:'a64'})), successors:[] }],
    });
    const r = decompile(model, { addr:BASE, name:'diag_o0', rowOfAddress, returnType:'int32', beginner:false });
    console.log('P3_O0', json({ ret:i(ir.instructions.find((x) => x.op === OP.RET)), memory:ir.instructions.filter((x) => x.op === OP.LOAD || x.op === OP.STORE).map(i), regions:pipeline.regions.map((x) => ({kind:x.kind,offset:x.offset,rootEntityId:x.rootEntityId,metadata:x.metadata})), semanticNodes:pipeline.semanticIr.nodes.map((x) => ({id:x.id,kind:x.kind,operator:x.operator,inputs:x.inputs,variable:x.variable,metadata:x.metadata,attributes:x.attributes?.machineEffects?.operationMetadata})), semantic:r.semantic, outputs:r.semanticAst?.outputs, inputs:r.semanticAst?.inputs, pseudocode:r.pseudocode }));
  }
  {
    ...
    const liftRows = fixture.model.instructions.map((decoded) => {
      let bundle = null, error = null;
      try {
        bundle = liftArm64MachineEffects(decoded, { instructionId:`diag-${decoded.row}`, origin:createOriginSet({instructionIds:[`diag-${decoded.row}`]}), mode:'a64' });
      } catch (e) { error = String(e?.message || e); }
      return { row:decoded.row, mnemonic:decoded.mnemonic, operands:decoded.operands, claimed:bundle != null, completeness:bundle?.completeness ?? null, unknownEffects:bundle?.unknownEffects ?? null, error };
    });
    ...
    console.log('P3_DECOMP', json({ semantic:r.semantic, legacyFallback:r.legacyFallback, warnings:r.warnings, unsupported:r.ctx?.semanticIRFallback?.unsupportedInstructions ?? r.ctx?.unknownInstructions, unknowns:r.ir?.instructions?.filter((x) => x.op === OP.UNKNOWN).map(i), liftRows }));
  }
} finally {
