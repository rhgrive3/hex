...
for(const [addr,mn,ops] of oracle.sampleInsns){const want=expected(mn,ops);if(!want)continue;const row=(addr-vm)/4;if(!Number.isInteger(row)||row<0||row>=w.scan.kindsCovered)continue;judged++;const got=names[w.scan.kinds[row]];if(got===want){hit++;continue;}const raw=await w.backend.readAt(BigInt(addr),4);const bytes=raw?.bytes;const word=bytes?.length>=4?new DataView(bytes.buffer,bytes.byteOffset,4).getUint32(0,true):null;bad.push({addr:'0x'+addr.toString(16),word:word==null?null:'0x'+word.toString(16).padStart(8,'0'),mn,ops,want,got});}
