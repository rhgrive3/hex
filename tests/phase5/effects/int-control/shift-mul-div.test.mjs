...
test('shift effective count zero proves complete state preservation', () => {
...
test('variable CL shift models count-zero as a conditional physical write', () => {
  ...
  assert.ok(operations(bundle,'value').some((operation) => operation.opcode === 'select' && operation.metadata.semantic === 'x86-conditional-register-write'));
  assert.equal(registerWrites(bundle,'rax').length, 1);
...
test('ROL/ROR affect only CF and count-dependent OF', () => {
...
test('IDIV records signed quotient/remainder contract and undefined arithmetic flags', () => {
  ...
  assert.equal(intrinsics(bundle,'x86.integer.idiv')[0].metadata.signed, true);
