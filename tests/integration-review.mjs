...
await test('planner does not let verification failure silently promote a weaker candidate', async () => {
  const A = 0x100000100n, B = 0x100000200n;
  const q = compileGoal('Increase coin');
  const facts = new Map([
    [A.toString(),[{kind:FACT.RMW,operation:'add',location:{key:'field:x0:32',disp:32n},evidence:['a']}]],
    [B.toString(),[{kind:FACT.RMW,operation:'add',location:{key:'field:x0:32',disp:32n},evidence:['b']}]],
  ]);
  const verificationTools = {
    search_functions: async () => ({results:[{address:A,name:'first'},{address:B,name:'second'}]}),
    search_strings: async () => ({results:[]}),
    get_callers: async () => ({results:[]}),
    get_callees: async () => ({results:[]}),
    get_xrefs: async () => ({results:[],functions:[]}),
    get_function: async (address) => ({address,name:address===A?'first':'second',found:true,instructions:1,cost:{functions:1,disassembly:1},summary:{calls:[]}}),
    get_semantic_facts: async (address) => ({results:facts.get(address.toString()) || []}),
    verify_field_update: async (address) => ({verified:address===B,evidence:address===B?['verified-b']:[]}),
    find_thresholds: async () => ({results:[]}),
  };
  const planned = await planAnalysisGoal(q,{}, {tools:verificationTools,maxFunctions:8,maxDisassembly:100,timeoutMs:1000});
  assert.equal(planned.best.address,B);
  assert.ok(planned.best.verification.verified);
});

await test('planner marks custom get_function results without explicit cost as unaccounted', async () => {
  const A = 0x100000100n;
  const q = compileGoal('Increase coin');
  const tools = {
    search_functions: async () => ({results:[{address:A,name:'coin'}]}),
    search_strings: async () => ({results:[]}),
    get_callers: async () => ({results:[]}),
    get_callees: async () => ({results:[]}),
    get_xrefs: async () => ({results:[],functions:[]}),
    get_function: async () => ({address:A,name:'coin',found:true,instructions:999,summary:{calls:[]}}),
    get_semantic_facts: async () => ({results:[]}),
    verify_field_update: async () => ({verified:false,evidence:[]}),
    find_thresholds: async () => ({results:[]}),
  };
  const planned = await planAnalysisGoal(q,{}, {tools,maxFunctions:8,maxDisassembly:100,timeoutMs:1000});
  assert.ok(planned.missingEvidence.includes('unaccounted-tool-cost'));
  assert.equal(planned.stats.disassembly,0);
});
