...
process.env.HEX_ANALYSIS_ROUTE = 'artifact';
...
class ImmediateWorker {
  ...
  postMessage(message) {
    ...
    if (message.t === 'analyze') queueMicrotask(() => this.onmessage?.({ data:{
      ...
      result:{ values:new BigUint64Array([1n,2n]), kinds:new Uint8Array([3,4]), big:0x1_0000_0000_0001n },
    }}));
  }
  terminate() {}
}
...
assert.equal(BACKEND_DEFAULT_ANALYSIS_ROUTE, ANALYSIS_ORCHESTRATION_ROUTE.CURRENT, 'rehearsal must not cut over production default');
...
assert.equal(backend.analysisRouteInfo().route, 'artifact', 'rehearsal env must force ambient artifact route');
assert.equal(backend.analysisRouteInfo().defaultCutover, false, 'production default must remain current during rehearsal');
...
console.log('PHASE4_PUBLIC_ARTIFACT_REHEARSAL ' + JSON.stringify({
  productionDefault:'current',
  forcedRoute:'artifact',
...
console.log('phase4 public artifact route rehearsal: PASS');
