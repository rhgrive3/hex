...
if (!input) {
  console.error('usage: node tests/ai-eval/grade.mjs <results.jsonl> [report.json]');
  process.exit(2);
}
...
const selectedDefs = defsDoc.cases.filter((d) => records.some((r) => r.caseId === d.id));
const missing = selectedDefs.filter((d) => !grades.some((g) => g.caseId === d.id)).map((d) => d.id);
