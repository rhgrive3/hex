...
# #799: signed absolute-value recognition requires a proven signed comparison.
...
# #801: fixed-width nested division is not generally associative.
...
# #806: reuse the established semantics-preserving MOV transparency helper.
...
# #810: null Itanium virtual entries are legal slots, not a table terminator.
...
const signedX = expr.variable('sx', 32, true);
const signedNeg = expr.unary('neg', signedX, 32, true);
const signedSel = expr.select(expr.compare('lt', signedX, zero, true), signedNeg, signedX, 32, true);
...
print('PR #884 semantic overlay materialized')
