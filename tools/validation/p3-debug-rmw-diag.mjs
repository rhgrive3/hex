...
function publicSummary(u){return {kind:u.kind,engine:u.engine,store:u.store,from:u.from,steps:(u.steps||[]).map(s=>({op:s.op,imm:s.imm==null?null:String(s.imm),row:s.row})),location:u.location?{key:u.location.key,irKey:u.location.irKey,disp:u.location.disp==null?null:String(u.location.disp),base:u.location.base}:null,register:u.register,helper:u.helper};}

{
 ...
 console.log('RMW_PUBLIC '+safe(updates.map(publicSummary)));
}
{
 ...
 console.log('ARC_PUBLIC '+safe(updates.map(publicSummary)));
}
