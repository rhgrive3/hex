...
s=p.read_text()
start=s.index('function mappedFileRangeForRva')
end=s.index('\nexport function parseImports', start)
helpers=r'''export const PE_METADATA_LIMITS = Object.freeze({
...
# Replace full exported parser functions by marker ranges.
def replace_fn(name,next_name,new):
    p=Path('js/binary/pe-loader.js'); t=p.read_text()
    a=t.index(f'export function {name}')
    b=t.index(f'\nexport function {next_name}',a) if next_name else len(t)
    p.write_text(t[:a]+new+t[b:])

replace_fn('parseImports','parseExports',r'''export function parseImports(r, dir, image, sharedBudget = null) {
  ...
  for (let guard = 0; guard < 65536 && off + 20 <= end; guard++, off += 20) {
    ...
    if(!terminated)markImportPartial(image,`PE import thunk table for ${library||'<unknown>'} reached its mapped file boundary/budget without a NUL terminator`);
  }
...
replace_fn('parseExports','parseExceptionFunctions',r'''export function parseExports(r, dir, image, sharedBudget = null) {
...
replace_fn('parseExceptionFunctions','parseBaseRelocations',r'''export function parseExceptionFunctions(r, dir, image, machine, sharedBudget = null) {
...
# Base relocation function ends before parseCoffSymbols but has helper allowedBaseRelocationTypes before it; replace only function body manually.
p=Path('js/binary/pe-loader.js');t=p.read_text();a=t.index('export function parseBaseRelocations');b=t.index('\nexport function parseCoffSymbols',a)
new=r'''export function parseBaseRelocations(r, dir, image, machine = null, sharedBudget = null) {
...
# COFF symbols shared budget.
p=Path('js/binary/pe-loader.js');t=p.read_text();a=t.index('export function parseCoffSymbols');b=t.index('\nexport function directory',a)
new=r'''export function parseCoffSymbols(r, ptr, count, image, sharedBudget = null) {
...
# Delay imports, TLS, LoadConfig are at end; replace each.
p=Path('js/binary/pe-loader.js');t=p.read_text();a=t.index('export function parseDelayImports');b=t.index('\nfunction readPointer',a)
new=r'''export function parseDelayImports(r, dir, image, sharedBudget = null) {
  ...
  for(let guard=0;guard<65536&&off+32<=end;guard++,off+=32){
    ...
    if(!terminated)budget.partial('delay-imports:unterminated-thunk',`PE delay-import thunk table for ${library} reached its mapped boundary/budget`);
  }
}
'''
p.write_text(t[:a]+new+t[b:])

p=Path('js/binary/pe-loader.js');t=p.read_text();a=t.index('export function parseTlsDirectory');b=t.index('\nexport function parseLoadConfig',a)
new=r'''export function parseTlsDirectory(r, dir, image, sharedBudget = null) {
...
'''
p.write_text(t[:a]+new)

# PE entrypoint file creates one shared budget and passes it to all decoders.
p=Path('js/binary/pe.js');s=p.read_text()
s=s.replace("import { parseImports, parseExports, parseExceptionFunctions, parseBaseRelocations, parseCoffSymbols, parseDelayImports, parseTlsDirectory, parseLoadConfig, resolveCoffSectionName, directory, peMachineName } from './pe-loader.js';","import { parseImports, parseExports, parseExceptionFunctions, parseBaseRelocations, parseCoffSymbols, parseDelayImports, parseTlsDirectory, parseLoadConfig, resolveCoffSectionName, directory, peMachineName, createPEMetadataBudget } from './pe-loader.js';",1)
s=s.replace('export function parsePE(input) {','export function parsePE(input, options = {}) {',1)
old="""  if (entryRva) seedValidatedEntrypoint(image, entryRva, sizeOfImage, machine);
...
new="""  if (entryRva) seedValidatedEntrypoint(image, entryRva, sizeOfImage, machine);
...
if old not in s: raise SystemExit('PE parser invocation anchor missing')
p.write_text(s.replace(old,new,1))

print('applied PE #535/#536 budget and mapped-span fixes')
