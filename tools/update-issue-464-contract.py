...
p=Path('tests/run.js'); text=p.read_text()
...
if new not in text:
    if old not in text: raise SystemExit('symbol contract anchor not found')
...
if new not in text:
    if old not in text: raise SystemExit('program contract anchor not found')
    text=text.replace(old,new,1)
p.write_text(text)
