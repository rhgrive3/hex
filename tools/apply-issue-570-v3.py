...
def sub_once(s, pattern, repl, label, flags=0):
    ...
    if n != 1:
        raise SystemExit(f'{label} regex matched {n}')
    return out


def function_slice(s, start_marker, next_marker):
    a=s.index(start_marker)
    b=s.index(next_marker,a)
    return a,b,s[a:b]

# Make ensure() truly parser-wide even for helpers that are called directly.
p=Path('js/binary/macho-budget.js'); s=p.read_text()
s=sub_once(s,
 r"export function ensureMachOMetadataBudget\(image, budget = null\) \{[\s\S]*?\n\}",
...
# If an earlier failed local patch never persists this still matches current main.
s=sub_once(s,
 r"(const image\s*=\s*new BinaryImage\([\s\S]*?\);\n)",
 r"\1  const metadataBudget = ensureMachOMetadataBudget(image);\n",
 'macho budget construction')
# Load-command work itself is decoded metadata too.
s=replace_once(s,
 ...
 'load command budget')
# Pass one shared object to every high-amplification decoder.
replacements={
...
# Budget the temporary name index and fallback function amplification.
...
a,b,part=function_slice(s,'function parseSymbolTable(', '\nfunction parseFunctionStarts')
part=sub_once(part,r"for \(let i = 0; i < st\.nsyms; i\+\+\) \{",
 "for (let i = 0; i < st.nsyms; i++) {\n    if (!budget.take({ inputBytes:ent, records:1, objects:1, operations:2, estimatedHeapBytes:224 }, 'symbol-record')) { image.metadata.symbolTable={complete:false,decoded:i,declared:st.nsyms,reason:'metadata-budget'}; break; }",'symbol record loop')
part=sub_once(part,r"(const name\s*=\s*strx < st\.strsize \? r\.cstring\([^\n]+\) : '';\n\s*if \(!name\) continue;)",r"\1\n    if (!budget.take({ stringBytes:name.length*2, estimatedHeapBytes:name.length*2+32 }, 'symbol-name')) break;",'symbol string budget')
...
a,b,part=function_slice(s,'function parseFunctionStarts(', '\nfunction dylibForOrdinal')
part=sub_once(part,r"while \(p < end\) \{", "while (p < end) {\n    if (!budget.take({ records:1, operations:1, estimatedHeapBytes:32 }, 'function-start-record')) { status.complete=false; status.partialReason='metadata-budget'; break; }",'function starts record')
...
a,b,part=function_slice(s,'export function parseChainedImports', '\nexport function parseChainedBindingSites')
part=sub_once(part,r"for\s*\(\s*let i\s*=\s*0\s*;\s*i\s*<\s*importsCount\s*;\s*i\+\+\s*\)\s*\{",
...
# String charge immediately after any successfully decoded import name.
...
a,b,part=function_slice(s,'export function parseChainedBindingSites', '\nfunction chainedPointerWidth')
...
part=sub_once(part,r"for \(let guard = 0; guard < 100000; guard\+\+\) \{", "for (let guard = 0; guard < 100000; guard++) {\n          if(!budget.take({inputBytes:width,records:1,operations:1,estimatedHeapBytes:16},'chained-pointer')){fail('shared metadata budget exhausted while decoding pointer chain');status.bindingSites=decoded;return status;}",'chained pointer loop')
...
a,b,part=function_slice(s,'export function parseClassicBindings', '\nexport function parseExportTrie')
...
# Limit repeated binds before the loop begins.
repeat_pat=r"if \(a\.value > 10_000_000n\) \{ fail\('bind repeat count exceeds budget'\); break; \}\s*for \(let i = 0n; i < a\.value; i\+\+\) \{ bind\(\); segOffset \+= ptrSize \+ b\.value; \}"
repeat_repl="""if(a.value>BigInt(Number.MAX_SAFE_INTEGER)){fail('bind repeat count exceeds safe integer range');break;}
...
part, n = re.subn(repeat_pat,repeat_repl,part,count=1)
if n!=1: raise SystemExit(f'classic repeat regex matched {n}')
...
a,b,part=function_slice(s,'export function parseExportTrie', '\nfunction normalizeSpecialDylibOrdinal')
...
# Budget each emitted export and executable function seed.
part=part.replace("image.exports.push({ name: prefix, address: 0n, kind: 'reexport'", "if(!budget.take({objects:1,operations:1,stringBytes:prefix.length*2,estimatedHeapBytes:prefix.length*2+160},'export-trie-output')){markPartial('shared metadata output budget exceeded','budgetExceeded');return;} image.exports.push({ name: prefix, address: 0n, kind: 'reexport'")
...
# Update source guards to accept parser-wide budget threading rather than the
# earlier prototype's exact formatting.
p=Path('tests/issue-570-macho-metadata-budget.mjs'); t=p.read_text()
t=t.replace("assert.match(macho,/const metadataBudget = createMachOMetadataBudget/);", "assert.match(macho,/const metadataBudget = ensureMachOMetadataBudget/);")
t=t.replace("assert.match(dyld,/export-trie-prefix/);", "assert.match(dyld,/export-trie-(node|output)/);")
p.write_text(t)
